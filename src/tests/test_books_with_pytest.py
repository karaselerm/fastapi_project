...
async def test_create_book(db_session, async_client):
    seller = sellers.Seller(first_name="Pushkin", last_name="Pushkinov", email="pushkin@mail.ru", password="pushkin123")
...
async def test_get_books(db_session, async_client):
    ...
    seller = sellers.Seller(first_name="Pushkin", last_name="Pushkinov", email="pushkin@mail.ru", password="pushkin123")
    seller2 = sellers.Seller(first_name="Рун", last_name="йцук", email="йцук@mail.ru", password="йцук")
    db_session.add_all([seller, seller2])
    await db_session.flush()

    book = books.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104, seller=seller)
    book_2 = books.Book(author="Lermontov", title="Mziri", year=1997, count_pages=104, seller=seller2)
    ...
    assert response.json() == {
        "books": [
            ...
            {"title": "Mziri", "author": "Lermontov", "year": 1997, "id": book_2.id, "count_pages": 104, "seller_id":seller2.id},
        ]
    }
...
async def test_update_book(db_session, async_client):
    # Создаем книги вручную, а не через ручку, чтобы нам не попасться на ошибку которая
    # может случиться в POST ручке
    # Создаем продавца вручную, чтобы иметь его ID для книги
    seller = sellers.Seller(first_name="Pushkin", last_name="Pushkinov", email="pushkin@mail.ru", password="pushkin123")
    db_session.add(seller)
    await db_session.flush()
